  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content move-dialog">
      <div class="modal-header">
        ...
        <h3 v-else-if="taskIds">Move {{ taskIds.length }} Tasks</h3>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary" 
                ...
                @click="onMove">Move {{ taskIds ? 'Tasks' : 'Task' }}</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Note: We don't easily know the current project of the task from props
    // but we can try to find it.
    const findTaskProject = () => {
        const targetId = props.task ? props.task.id : (props.taskIds?.length > 0 ? props.taskIds[0] : null);
        if (!targetId) return props.projects[0]?.id;

        for (const p of props.projects) {
            for (const l of p.lists) {
                if (l.tasks.some(t => t.id === targetId)) return p.id;
            }
        }
        return props.projects[0]?.id;
    };

    selectedProjectId.value = findTaskProject();
...
.list-option.selected {
  background: var(--accent-blue-transparent, rgba(0, 120, 215, 0.2));
  border-color: var(--accent-blue);
}
